...
template<long S>
std::array<long, S> valSort(std::array<long, S> arr)
{
	for (unsigned long long i = 1; i < S; ++i) { // вставкой desc
...
template<long S>
void ptrSort(std::array<long, S>* arr)
{
...
template<long S>
void refSort(std::array<long, S>& arr)
{
	for (unsigned long long i = 1; i < S; ++i) {
		for (unsigned long long j = i - 1; j >= 0; --j) {
			if (arr[j] >= arr[j + 1]) break;
